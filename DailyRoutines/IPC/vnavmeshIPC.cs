    ...
    public static void Init()
    {
        if (Utils.HasPlugin(Name))
        {
            try
            {
                ...
                _navReload = Service.PluginInterface.GetIpcSubscriber<bool>($"{Name}.Nav.Reload");
                _navRebuild = Service.PluginInterface.GetIpcSubscriber<bool>($"{Name}.Nav.Rebuild");
                _navPathfind = Service.PluginInterface.GetIpcSubscriber<Vector3, Vector3, bool, Task<List<Vector3>>>($"{Name}.Nav.Pathfind");
                ...
                _queryMeshPointOnFloor = Service.PluginInterface.GetIpcSubscriber<Vector3, float, Vector3?>($"{Name}.Query.Mesh.PointOnFloor");

                _pathMoveTo = Service.PluginInterface.GetIpcSubscriber<List<Vector3>, bool, object>($"{Name}.Path.MoveTo");
                ...
                _pathfindInProgress = Service.PluginInterface.GetIpcSubscriber<bool>($"{Name}.SimpleMove.PathfindInProgress");
            }
